...
def get_args():
    parser = argparse.ArgumentParser(description="Single prompt image generation")
    parser.add_argument("--prompt", type=str, required=True, help="Text prompt for image generation")
    parser.add_argument("--save-path", type=str, default="./generated_image.jpg", help="Path to save generated image")
    ...
    parser.add_argument("--data-type", type=str, default="generation", help="Data type for model")
...
def save_images(imgs, save_path, prompt):
    ...
    if isinstance(imgs, dict):
        ...
        if vq_images is not None and len(vq_images) > 0:
            ...
            print_with_time(f"VQ image saved to: {vq_save_path}")
        ...
        if diff_images is not None and len(diff_images) > 0:
            ...
            print_with_time(f"Diff image saved to: {diff_save_path}")
    else:
        if isinstance(imgs, list) and len(imgs) > 0:
            imgs[0].save(save_path)
            print_with_time(f"Image saved to: {save_path}")
        else:
...
def main():
    ...
    print_with_time("Starting image generation...")
    imgs = generate_single_image(model, args.prompt, args)
    
    if imgs is not None:
        save_images(imgs, args.save_path, args.prompt)
        print_with_time("Image generation completed!")
    else:
        print_with_time("Image generation failed!")
